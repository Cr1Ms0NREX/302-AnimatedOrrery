...
public class OrbitPlanets : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!orbitCenter) return;
        {
            ...
            //float y = radius * Mathf.Tan(Time.time * timeMultiplier + timeOffset);
            transform.position = new Vector3(x, 0, z) + orbitCenter.position;

            // First take on controlling the time of the orbits;
